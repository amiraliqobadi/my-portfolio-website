...
const destinations = [
    { id: "certificates", panel: "certificates", x: 870, y: 900, color: "#9b6dd6", number: "03" },
    { id: "about", panel: "about", x: 1100, y: 720, color: "#43a878", number: "01" },
    { id: "work", panel: "work", x: 1560, y: 900, color: "#e9a23b", number: "02" },
    { id: "contact", panel: "contact", x: 1970, y: 900, color: "#4e8fd1", number: "04" },
];

const destinationCopy = {
    en: {
        about: { label: "ABOUT", title: "About Amirali", body: "Background, stack and engineering philosophy." },
        work: { label: "WORK", title: "Selected work", body: "Five real products live in production." },
        certificates: { label: "CREDENTIALS", title: "Certificates", body: "ICPC contests and 400 hours of Django training." },
        contact: { label: "CONTACT", title: "Let's talk", body: "Email, GitHub and LinkedIn are one click away." },
    },
    fa: {
        about: { label: "درباره", title: "درباره امیرعلی", body: "پیشینه، استک و فلسفه مهندسی." },
        work: { label: "پروژه‌ها", title: "کارهای منتخب", body: "پنج محصول واقعی که آنلاین هستند." },
        certificates: { label: "گواهینامه‌ها", title: "مدارک و افتخارات", body: "مسابقات ICPC و ۴۰۰ ساعت آموزش جنگو." },
        contact: { label: "ارتباط", title: "بیایید صحبت کنیم", body: "ایمیل، گیت‌هاب و لینکدین یک کلیک فاصله دارند." },
    },
};

const copy = {
    en: { kicker: "FULL-STACK ENGINEER · INTERACTIVE PORTFOLIO", titleA: "Hi, I'm Amirali.", titleB: "I build digital worlds.", intro: "Drive around, discover my work and park beside a project or signpost to open it.", start: "Start exploring", controls: "Use WASD or arrow keys to drive", nearby: "Project discovered", destinationFound: "Destination found", visit: "Visit live project", openSection: "Open section", about: "About", work: "Work", certificates: "Certificates", contact: "Contact", reset: "Reset car", aboutTitle: "Engineer first, coder second.", aboutBody: "I'm a full-stack engineer specializing in Python backends and React frontends. I design and ship complete products—from database architecture to the last interaction on screen.", aboutStat1: "5 live platforms", aboutStat2: "270+ problems solved", certificatesTitle: "Credentials earned through practice.", certificatesBody: "Competitive programming and 400 hours of practical Python and Django training.", openCertificate: "Open full certificate", contactTitle: "Let's make something memorable.", contactBody: "I'm open to ambitious product work, full-stack roles and select freelance projects.", tip: "Drive close to a signpost or project station" },
    fa: { kicker: "مهندس فول‌استک · پورتفولیوی تعاملی", titleA: "سلام، من امیرعلی‌ام.", titleB: "دنیای دیجیتال می‌سازم.", intro: "رانندگی کن، پروژه‌هایم را پیدا کن و کنار هر پروژه یا تابلو توقف کن تا باز شود.", start: "شروع ماجراجویی", controls: "با WASD یا کلیدهای جهت حرکت کن", nearby: "پروژه پیدا شد", destinationFound: "مقصد پیدا شد", visit: "مشاهده پروژه", openSection: "باز کردن بخش", about: "درباره من", work: "پروژه‌ها", certificates: "گواهینامه‌ها", contact: "ارتباط", reset: "بازگشت ماشین", aboutTitle: "اول مهندس، بعد برنامه‌نویس.", aboutBody: "مهندس فول‌استک متخصص در بک‌اند پایتون و فرانت‌اند React هستم. محصول را از معماری دیتابیس تا آخرین تعامل روی صفحه طراحی و منتشر می‌کنم.", aboutStat1: "۵ پلتفرم آنلاین", aboutStat2: "۲۷۰+ مسئله حل‌شده", certificatesTitle: "اعتبارهایی که با تمرین به دست آمده‌اند.", certificatesBody: "مسابقات برنامه‌نویسی و ۴۰۰ ساعت آموزش عملی پایتون و جنگو.", openCertificate: "مشاهده کامل گواهینامه", contactTitle: "بیایید چیزی ماندگار بسازیم.", contactBody: "برای پروژه‌های جدی، موقعیت‌های فول‌استک و همکاری فریلنس آماده‌ام.", tip: "به تابلوها یا ایستگاه‌های رنگی پروژه نزدیک شو" },
};
...
const clouds = Array.from({ length: 10 }, (_, i) => ({ x: 110 + ((i * 431) % 2480), y: 110 + ((i * 233) % 1520), r: 34 + (i % 4) * 10 }));

function roundedRect(ctx, x, y, width, height, radius) {
    ctx.beginPath(); ctx.roundRect(x, y, width, height, Math.min(radius, width / 2, height / 2));
}

function drawDestination(ctx, destination, info, time) {
    const bob = Math.sin(time / 420 + destination.x) * 2.5;
    ctx.save();
    ctx.translate(destination.x, destination.y + bob);
    ctx.fillStyle = "rgba(20,30,20,.20)";
    ctx.beginPath(); ctx.ellipse(0, 7, 56, 18, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#2d2d27"; ctx.fillRect(-6, -64, 12, 64);
    ctx.fillStyle = destination.color; roundedRect(ctx, -66, -118, 132, 64, 16); ctx.fill();
    ctx.fillStyle = "#f7f1e4"; roundedRect(ctx, -58, -110, 116, 48, 10); ctx.fill();
    ctx.fillStyle = destination.color; ctx.beginPath(); ctx.arc(-38, -86, 17, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#fff"; ctx.font = "800 11px system-ui"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText(destination.number, -38, -86);
    ctx.fillStyle = "#24251f"; ctx.font = "800 12px system-ui"; ctx.textAlign = "left"; ctx.fillText(info.label, -12, -86);
    ctx.restore();
}

function drawWorld(ctx, width, height, dpr, car, images, time, lang) {
    ...
    clouds.forEach((cloud) => { const x = (cloud.x + time * .008) % (WORLD.width + 240) - 120; ctx.fillStyle = "rgba(255,255,255,.16)"; ctx.beginPath(); ctx.ellipse(x, cloud.y, cloud.r, cloud.r * .48, 0, 0, Math.PI * 2); ctx.ellipse(x + cloud.r * .55, cloud.y - 8, cloud.r * .62, cloud.r * .32, 0, 0, Math.PI * 2); ctx.ellipse(x - cloud.r * .55, cloud.y - 4, cloud.r * .55, cloud.r * .3, 0, 0, Math.PI * 2); ctx.fill(); });
    ...
    destinations.forEach((destination) => drawDestination(ctx, destination, destinationCopy[lang][destination.id], time));
...
export default function InteractivePortfolio() {
    ...
    const travelTo = useCallback((project) => { const next = { x: project.x, y: project.y + 205, angle: 0, speed: 0 }; car.current = next; setMapPosition(next); setNearby({ type: "project", ...project }); setPanel(null); setStarted(true); }, []);
    const travelToDestination = useCallback((destination) => { const next = { x: destination.x, y: destination.y + 165, angle: 0, speed: 0 }; car.current = next; setMapPosition(next); setNearby({ type: "destination", ...destination }); setPanel(null); setStarted(true); }, []);
    ...
    useEffect(() => {
        ...
        const loop = (now) => { const delta = Math.min((now - previous) / 16.67, 2); previous = now; const state = car.current; const forward = keys.current.ArrowUp || keys.current.KeyW; const backward = keys.current.ArrowDown || keys.current.KeyS; const left = keys.current.ArrowLeft || keys.current.KeyA; const right = keys.current.ArrowRight || keys.current.KeyD;
            ...
            const nearbyPoints = [...projects.map((project) => ({ type: "project", item: project, distance: Math.hypot(state.x - project.x, state.y - project.y) })), ...destinations.map((destination) => ({ type: "destination", item: destination, distance: Math.hypot(state.x - destination.x, state.y - destination.y) }))]; const closest = nearbyPoints.sort((a, b) => a.distance - b.distance)[0]; const nextNearby = closest.distance < (closest.type === "project" ? 210 : 145) ? { type: closest.type, ...closest.item } : null; setNearby((current) => current?.id === nextNearby?.id && current?.type === nextNearby?.type ? current : nextNearby); setMapPosition((current) => Math.abs(current.x - state.x) > 10 || Math.abs(current.y - state.y) > 10 ? { x: state.x, y: state.y } : current); drawWorld(ctx, window.innerWidth, window.innerHeight, Math.min(window.devicePixelRatio || 1, 2), state, images, now, lang); frame = requestAnimationFrame(loop); };
    ...
    return <main className="world-shell" dir={lang === "fa" ? "rtl" : "ltr"}>
        ...
        {!started && <section className="welcome-card"><div className="welcome-kicker"><Sparkles size={14} /> {c.kicker}</div><h1>{c.titleA}<br /><em>{c.titleB}</em></h1><p>{c.intro}</p><div className="welcome-actions"><button className="primary-cta" onClick={() => setStarted(true)}>{c.start}<ArrowRight size={18} /></button><button className="secondary-cta" onClick={() => { setStarted(true); setPanel("certificates"); }}><Sparkles size={15} />{c.certificates}</button><button className="secondary-cta" onClick={() => { setStarted(true); setPanel("contact"); }}><Mail size={15} />{c.contact}</button></div><div className="control-hint"><span className="key-set">W A S D</span>{c.controls}</div></section>}
        {started && !panel && <div className="explore-tip"><MousePointer2 size={15} /><span>{c.tip}</span></div>}
        {nearby?.type === "project" && started && !panel && <article className="project-popover" style={{ "--accent": nearby.color }}><div className="project-image"><img src={nearby.image} alt="" /></div><div className="project-copy"><span className="discovered"><i />{c.nearby}</span><p>{nearby.eyebrow}</p><h2>{nearby.title}</h2><div className="project-tags">{nearby.tech.map((item) => <span key={item}>{item}</span>)}</div><a href={nearby.url} target="_blank" rel="noreferrer">{c.visit}<ExternalLink size={15} /></a></div></article>}
        {nearby?.type === "destination" && started && !panel && <article className="project-popover destination-popover" style={{ "--accent": nearby.color }}><div className="destination-banner">{nearby.number}</div><div className="project-copy"><span className="discovered"><i />{c.destinationFound}</span><p>{destinationCopy[lang][nearby.id].title}</p><h2>{destinationCopy[lang][nearby.id].label}</h2><p className="destination-description">{destinationCopy[lang][nearby.id].body}</p><button className="popover-action" onClick={() => setPanel(nearby.panel)}>{c.openSection}<ArrowRight size={15} /></button></div></article>}
        <aside className="map-card"><div className="map-label"><Map size={14} /><span>WORLD MAP</span></div><div className="mini-map"><div className="road horizontal" /><div className="road vertical one" /><div className="road vertical two" />{projects.map((project) => <button key={project.id} style={{ left: `${project.x / WORLD.width * 100}%`, top: `${project.y / WORLD.height * 100}%`, background: project.color }} onClick={() => travelTo(project)} aria-label={`Go to ${project.title}`} />)}{destinations.map((destination) => <button key={destination.id} className="destination" style={{ left: `${destination.x / WORLD.width * 100}%`, top: `${destination.y / WORLD.height * 100}%`, borderColor: destination.color }} onClick={() => travelToDestination(destination)} aria-label={destinationCopy[lang][destination.id].title} />)}<i style={{ left: `${mapPosition.x / WORLD.width * 100}%`, top: `${mapPosition.y / WORLD.height * 100}%` }} /></div><button className="reset-link" onClick={resetCar}><RotateCcw size={12} />{c.reset}</button></aside>
